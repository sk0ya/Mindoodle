...
### 段階的移行ステップ（低リスク順）
1) Panel/Mode の導入
   - `uiSlice` 内に `panelManager`/`uiModeMachine` を実装（既存 boolean は段階的にデリゲート）。
   - 画面の可視制御を Panel API 経由へ差し替え。
2) Command 統一
   - `CommandContext`/`guard`/`execute` を導入。
   - ショートカット/Vim/パレット/メニュー/ボタンからの呼び出しを `execute` に統一。
3) Controller 抽出
   - `MindMapApp.tsx` からロジックを `MindMapController` へ移行し、View を薄く。
4) イベント Strategy 化
   - Canvas のイベントを `EventStrategy`（Normal/Insert→Visual の順）で差し替え。
5) Service 化
   - `Navigation/Viewport/Clipboard/Workspace` の重複ロジックをサービスへ移し、Hook は配線のみ。
6) Selector 整備
   - 行番号解決/親子探索/空間探索などの共通処理を `selectors` へ集約。
7) グローバルブリッジの解消
   - `(window as any)` 経由の API を Controller/Command に置換。

### 期待効果
- 分岐削減: 入力条件のガードが Command/Panel/Mode に集約し、UI レイヤの if/else が大幅減。
- 一貫性: 入力→Command→Service→Store の流れが明確化。副作用の配置が一元化。
- テスト容易性: Controller/Service/Selector は単体テスト可能。UI は描画検証に集中。
- 変更影響の局所化: パネル排他やモード遷移不具合は Panel/Mode に閉じた修正で解決。

### 実装メモ（型とスタイル）
- Strict TS 維持。`UIMode`/`PanelId` は判別可能ユニオンで定義。
- 既存のパスエイリアスと `src/app/index.ts` の再エクスポート方針を維持。
- 既存 `commands` 構成を踏襲し guard/execute のみ拡張。
